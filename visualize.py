...
from utils.pose_utils import visualize_depth, visualize_normals, to8b
...
def visualize(config):
    ...
    imageio.mimwrite(path.join(config.save_dir, "video.mp4"), rgb_frames, fps=30, quality=10)# , codecs="hvec"
    if config.visualize_depth:
        imageio.mimwrite(path.join(config.save_dir, "depth.mp4"), depth_frames, fps=30, quality=10)
    if config.visualize_normals:
        imageio.mimwrite(path.join(config.save_dir, "normals.mp4"), normal_frames, fps=30, quality=10)
