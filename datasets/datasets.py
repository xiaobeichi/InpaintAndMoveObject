...
import torch
from utils.ray_utils import Rays, convert_to_ndc, namedtuple_map
from utils import camera_utils
from utils.pose_utils import normalize, look_at, poses_avg, recenter_poses, to_float, generate_spiral_cam_to_world, generate_spherical_cam_to_world, flatten
...
def get_dataset(dataset_name, base_dir, split, factor=4, device=torch.device("cpu")):
    print("dataset_class: ", dataset_name)
...
class NeRFDataset(Dataset):
    def __init__(self, base_dir, split, spherify=False, near=2, far=6, white_bkgd=False, factor=1, n_poses=120, radius=None, radii=None, h=None, w=None, device=torch.device("cpu")):
        ...
        self.images = None
        self.load()
        #print("n_image: ", self.images.shape[0])

    def load(self):
        if self.split == "render":
            self.generate_render_rays()
        elif self.split == "test_image":
            self.generate_test_rays()
        else:
    ...
    def generate_test_rays(self):
        """
        Generates rays used to render a video using a trained mip-NeRF
        """
        print("Generating test Poses")
        self.generate_test_poses()
        print("Generating rays")
        self.generate_rays()
    ...
    def generate_rays(self):
        ...
        camera_directions = np.stack(
            [(x - self.w * 0.5 + 0.5) / self.focal,
             ...
             -np.ones_like(x)],
            axis=-1) #(w,h,3)
        # Rotate ray directions from camera frame to the world frame (n_train,w,h,3)
        ...
        #print("ones: ",ones.shape, "h: ", self.h, "w: ", self.w)
        #print("indice: ", np.tile(self.indices, (1,self.h,self.w,1)).shape)
        if self.split == "train":
            self.indices = np.linspace(0,self.images.shape[0]-1,num=self.images.shape[0],dtype=int)
            self.rays = Rays(
                origins=origins,
                ...
                near=ones * self.near,
                far=ones * self.far,
                indice=np.tile(self.indices, (self.h,self.w,1)).reshape(-1,self.h,self.w,1),
                isMask=(np.mean(self.images_mask, -1, keepdims=True)>(200/255)))
        else:
            self.rays = Rays(
                origins=origins,
                directions=directions,
    ...
    def __getitem__(self, i):
        ...
        if self.split == "render" or self.split == "test_image":
            # render rays
            #print(ray.shape)
    ...
    def __len__(self):
        if self.split == "test_images":
            ...
            #print("train len: ", len(self.images), " rays: ", self.rays.shape)
...
class LLFF(NeRFDataset):
    ...
    def generate_training_poses(self):
        ...
        if self.split == "train":
            for mask_file in mask_files:
                with open(mask_file, 'rb') as img_mask:
                    image = to_float(np.array(Image.open(img_mask)))
                    images_mask.append(image)
            images_mask = np.stack(images_mask, -1)
            images_mask = np.moveaxis(images_mask, -1, 0)
    ...
    def generate_render_poses(self):
        self.generate_training_poses()
        self.n_poses = 120#self.n_poses_copy  # get overwritten in generate_training_poses, change back to original
        if self.spherify:
            self.generate_spherical_poses(self.n_poses)
        else:
           self.generate_spiral_poses(self.n_poses)
    ...
    def generate_test_poses(self):
        self.generate_training_poses()
        #print("pose_copy: ", self.n_poses_copy)
        #self.n_poses = self.n_poses_copy
        indices = np.arange(self.images.shape[0])[::8]
        self.n_poses = indices.shape[0]
        print("n_pose: ",self.n_poses)
    ...
    def generate_training_rays(self):
        print("Loading Training Poses")
        self.generate_training_poses()
        if self.split == "train": # 0,8,... --> test image
            indices = [i for i in np.arange(self.images.shape[0]) if i not in np.arange(self.images.shape[0])[::8]]
        else:
            indices = np.arange(self.images.shape[0])[::8]
        self.images = self.images[indices]
        if self.split =="train":
            self.images_mask = self.images_mask[indices]
        #self.n_poses = len(indices)
    ...
    def generate_rays(self):
        ...
        super().generate_rays()
        #print("origin:",self.rays.origins)
        ...
        #print("mask:", np.mean(self.images_mask, -1).shape)
        ones = np.ones_like(ndc_origins[..., :1])
        if self.split == "train":
            self.indices = np.linspace(0,self.images.shape[0]-1,num=self.images.shape[0])
            print("indice: ",np.tile(self.indices, (self.h,self.w,1)).reshape(-1,self.h,self.w,1).shape)
            self.rays = Rays(
                origins=ndc_origins,
